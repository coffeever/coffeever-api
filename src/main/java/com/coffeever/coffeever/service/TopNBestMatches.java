...
public class TopNBestMatches {
    ...
    public List<CoffeeMerged> bestMatch (CoffeeMerged pref, Iterable<CoffeeMerged> allBeans){
        ...
        List<CoffeeMerged> bestMatches = new ArrayList<CoffeeMerged>();
        int n = 5;                                                        // Amount of item to be returned
        double gap = 22;                                                  // Max difference possible
        for(int i=0; i<n; i++){
    ...
    private double euclidean(CoffeeMerged b1, CoffeeMerged b2){
        String[] b1_keys = b1.getKeywords().replaceAll("\\s+","").toLowerCase().split(",");
        String[] b2_keys = b2.getKeywords().replaceAll("\\s+","").toLowerCase().split(",");
        int count = 0;
